...
export class PatientSymptomsInfo {
  ...
  temperature: number;
  inContactWithCovidPositivePatient: number =0;
  dateCreated: Date;
  ...
  get inContactWithCovidPositivePatientBool() {
    return this.inContactWithCovidPositivePatient == 1;
  }

  set inContactWithCovidPositivePatientBool(newValue: boolean) {
    this.inContactWithCovidPositivePatient = newValue ? 1 : 0;
  }  
}
...
export class PatientListModel {
  ...
  age: number;
  bloodGroup: string;
  hasHypertension: number = 0;
  hasDiabetes: number = 0;
  hasCardiovascularDisease: number = 0;
  hasChronicRespiratoryDisease: number = 0;
  registeredOn: Date;
}

export class PatientDailyRecordResponse {
  message: string;
  success: boolean;
}
